...
const EventsReport = () => {
  ...
  const excludedFields = ["deviceId", "attributes"];
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const url = `${apiURL}/reports/events?deviceId=${deviceId}&from=${from}&to=${to}`;
      ...
      const formattedData = jsonData.map((item) => ({
        ...item,
        eventTime: new Date(item.eventTime).toLocaleString(undefined, {
          year: "numeric",
          month: "short",
          day: "numeric",
          hour: "2-digit",
  ...
  return (
    <div className="flex-1 flex flex-col">
      <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
        <h1 className="text-2xl font-semibold text-gray-800">Events Report</h1>
        <p className="text-gray-600 text-sm">
          Generate Events report for your devices
        </p>
      </header>
        ...
        {generatedReports.length === 0 ? (
          ...
          <div className="bg-white shadow overflow-x-auto sm:rounded-lg my-10">
            <div className="px-4 py-5 sm:p-6">
              ...
              <div className="w-full mt-6 flex justify-end items-center space-x-2">
                <Button
                  variant="outline"
                  onClick={() =>
